...
class ProductItem(mongoengine.Document):
    product_id = mongoengine.ReferenceField(
...
class BasketItem(mongoengine.EmbeddedDocument):
    product_item_id = mongoengine.ReferenceField(ProductItem)
...
class User(mongoengine.Document):
    ...
    basket = mongoengine.EmbeddedDocumentListField(document_type=BasketItem)
...
class Driver(mongoengine.Document):
    ...
    last_name = mongoengine.StringField(max_length=64, required=True)
    telephone = mongoengine.StringField(max_length=32)
    license_plate = mongoengine.StringField(max_length=16)
...
class Order(mongoengine.Document):
    user_id = mongoengine.ReferenceField(
...
class Delivery(mongoengine.Document):
    ...
    driver_id = mongoengine.ReferenceField(
        Driver, reverse_delete_rule=mongoengine.NULLIFY
    )
